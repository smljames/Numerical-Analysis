...
class MAT {
    ...
    friend double det(MAT &A); // determinant of A
    friend MAT inv(MAT &A); // inverse of A
    ...
    friend double polyval(VEC &p, double x);
    friend VEC polydiff(VEC &p);
};
...
double det(MAT &A); // determinant of A
MAT inv(MAT &A);
...
double newton(double(*func)(double), double(*func_)(double), double x, double step, double tol, int maxiter); // newton' method to find the root of non-linear equation
double polyval(VEC &p, double x);
VEC polydiff(VEC &p);
VEC polyroot(VEC &p, double x, double tol, int maxiter);

void cj(VEC(*F)(VEC&), VEC &x, double h, int p, double tol, int maxiter); // Cyclic Jacobian Updates method
#endif
