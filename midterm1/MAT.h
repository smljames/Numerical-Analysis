...
class MAT {
    ...
    friend MAT &read_matrix(const char* file, MAT &A, int &dim);
    ...
    friend int EVpwr(MAT &A, VEC &q0, double &lambda, double tol, int maxiter, int method);
    friend int EVpwr_all_err(MAT &A, VEC &q0, VEC &lambda, int maxiter, int method, VEC &all_err);
    friend int EVipwr(MAT&A, VEC &q0, double &lambda, int maxiter);
};
MAT operator*(double a,MAT &m1); // dbl x m
VEC operator*(VEC &v1,MAT &m1); // vT x m
MAT read_matrix(const char* file, int &dim);
void GE(MAT A, VEC b);
...
int EVpwr(MAT &A, VEC &q0, double &lambda, double tol, int maxiter, int method);
int EVpwr_all_err(MAT &A, VEC &q0, VEC &lambda, int maxiter, int method, VEC &all_err);
int EVipwr(MAT&A, VEC &q0, double &lambda, int maxiter);
#endif
